...
#Load the models, These can be found on the GitHub
...
model.pics.Ross <- function(location_of_files="for_phenology", file_output="ML_results", i=1,j=500){
 ...
 # will read in ta list of the files that are within the folder, or write a list if there isn't one to save time. 
  ...
  # Note: The code is set up to work though through everything by species
  #       This will allow it to be re-run in species batches if there are any problem.
  ...
  for (sp_name in uniq_sp_names[i:j]) {
    ...
    # Saves a list of the files names for a given species. 
    ...
    for (k in sp_file_names) {
      ...
      if (class(tt)!="try-error") {
        ...
        x <- x/255 # converts the RGB colours to a vlaues between 0 and 1.
        ...
        # if the First flwoering result is greater than 50 it will then run the second model
